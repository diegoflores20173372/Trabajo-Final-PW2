...
<body>
	<div class="jumbotron boxlogin">
		<form method="post" action="/ballots/add" name="Reci" if="flogin">
			...
			<%for(String l : largo){ %>
				<option value="<%=l %>" <%if(l.equals(aux.getMedida().substring(0, aux.getMedida().indexOf("/")))){ %> autofocus <%} %>><%=l %></option>
				<%} %>
			</select>
			/
			<select name="largo" class="selectmod" >
			<%for(String an : ancho){ %>	
				<option value="<%=an %>" <%if(an.equals(aux.getMedida().substring(aux.getMedida().indexOf("/")))){ %> autofocus <%} %>><%=an %></option>
				<%} %>
			</select>
			R
			<select name="aro" class="selectmod" >
			<%for(String ar : aro){ %>	
				<option value="<%=ar %>" <%if(ar.equals(aux.getAro())){ %> autofocus <%} %>><%=ar %></option>
				<%} %>
			</select>
